...
const Navbar = () => {
  ...
  const menuRef = useRef(null); // ✅ for click outside
  ...
  return (
    <nav
      className={`w-full z-50 transition-all duration-500 shadow-lg ${
        dark
          ? "bg-gray-800/80 text-white border border-orange-500 rounded-2xl hover:shadow-yellow-400/30"
          : "bg-gray-100/70 text-gray-900 border border-blue-400 rounded-2xl"
      }`}
    >
      {/* Container */}
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleMenus} className="lg:hidden text-2xl">
            {isOpen ? (
              ...
              <RxHamburgerMenu className="border rounded-lg p-1 text-3xl" />
            )}
          </button>
        </div>
      </div>
      ...
      <div
        ref={menuRef}
        className={`fixed top-0 right-0 z-40 h-screen w-2/3 transform transition-transform duration-500 ease-in-out lg:hidden flex flex-col items-center py-5 gap-8 ${
          dark
