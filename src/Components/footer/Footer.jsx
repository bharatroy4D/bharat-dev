    ...
    <footer className={`${bgClass} ${borderColor} p-8 transition-all duration-500`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-center items-center gap-2 mb-4">
          <span className={theme === "dark" ? "text-lime-400" : "text-teal-600"}>&lt;/&gt;</span>
          <span className="font-semibold">Bharat</span>
          <span className="text-gray-400 font-semibold">roy</span>
        </div>

        {/* Social Media Icons */}
        <div className="flex justify-center items-center gap-4 mb-2 md:mb-5">
          <a href="#" className={`p-2 rounded transition-colors ${bgClass}`}>
            <Github className={`w-6 h-6 ${iconColor} transition-colors`} />
          </a>
          <a href="#" className={`p-2 rounded transition-colors ${bgClass}`}>
            <Linkedin className={`w-6 h-6 ${iconColor} transition-colors`} />
          </a>
          {/* Dev.to */}
          <a href="#" className={`p-2 rounded transition-colors ${bgClass}`}>
            <svg className={`w-6 h-6 ${iconColor} transition-colors`} viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="flex justify-center items-center gap-8 md:mb-5 flex-wrap">
          {["about","resume","services","portfolio","blog","contact"].map((link) => (
            <a key={link} href={`#${link}`} className={`text-sm transition-colors ${textHoverClass}`}>
        ...
        {/* Copyright */}
        <div className={`${theme === "dark" ? "text-gray-400" : "text-gray-600"} text-center text-sm`}>
